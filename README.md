...
마크업 시 시맨틱 태그로 레이아웃을 구성하는 것이 웹접근성과 검색엔진 최적화 및 유지보수를 위해 중요하다는 점을 알게 된 프로젝트
...
모바일 사이트 제작시 px 외에도 상대 단위(%, em, rem)를 이해하고, 미디어쿼리를 사용한 해상도 서비스 처리 작업을 고민해봤던 프로젝트
...
브라우저 창 조절에 따른 이미지 높이 처리 및 그리드, 미디어 쿼리를 활용해 디바이스에 따라 UI가 변하는 반응형 사이트를 제작하는 법을 익혔던 프로젝트
...
부트스트랩과 활용해 시간을 단축했고, 단점은 규칙과 기본 스타일 적용되어 있어 수정하기 번거로웠던 프로젝트

### 5. 워드프레스 반응형웹 프로젝트, 크로우캐(주방용품 브랜드)
 + https://earnestga3.cafe24.com/wordpress
 + 대표적 콘텐츠 관리 시스템(CMS)인 워드프레스를 이용한 반응형 웹사이트 제작

 + 오픈소스 CMS 중 가장 많이 사용하는 워드프레스의 Sydney 테마와, elementor 플러그인을 사용해 반응형 웹사이트 구현

워드프레스를 활용해 시간을 단축했으며, 이벤트 구현 시 자바스크립트 사용 없이도 간단하게 처리가 가능해 유지보수가 편하다고 느낀 프로젝트
...
컴포넌트 구성
 + header, footer, main, sub content를 컴포넌트로 구성해 재활용 및 유지보수를 고려
...
컴포넌트 사용으로 불필요한 코드의 재반복없이 컴포넌트를 재사용할 수 있는 점이 신기했고, 라우터 사용으로 비동기 방식의 처리가 가능하다는 것을 알 수 있었던 프로젝트
